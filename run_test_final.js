...
function isStarted(url, callback){
	console.log('He entrado en is started')
	request.get({
		url: url
...
function action(message){
	...
	if(containslevantaArm){
		...
		//var url='https://tjbotprueba.mybluemix.net';

		//isStarted(url, function(body){
			//started = body;
			//console.log(started);
		//});
		...
		tj.speak("¡Buena idea Antonio! Voy arrancando la competición. Te avisaré cuando esté lista.").then (function() {
			return tj.sleep(85000);
		}).then(function() {
			return tj.speak("Un momento Antonio, ya puedes empezar la competición.");
		}).then(function() {
			return tj.speak("Mi trabajo aquí ha terminado. Diles que vengan luego al estand de IBM. ¡Allí estaré! ¡Gracias ayudante! Ha sido un placer trabajar contigo");
		});
